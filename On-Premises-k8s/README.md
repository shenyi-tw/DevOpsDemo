# On-Premises-k8s RKE2 HA 負載均衡架構圖

基於 On-Premises（地端） 的 RKE2 Kubernetes 高可用（HA）集群 與 負載均衡（Load Balancer） 架構設計與驗證紀錄。

```txt
[ Client / Worker Nodes ]
         │
         ├─── Port 6443 (Kubernetes API) ────┐
         └─── Port 9345 (RKE2 Node Join) ────┤
                                             ▼
                        ┌────────────────────────────────────────┐
            ...
            ▼                              ▼                              ▼
┌──────────────────────────────┐┌──────────────────────────────┐┌──────────────────────────────┐
│        rke2-cp1              ││        rke2-cp2              ││        rke2-cp3              │
│  IP: 192.168.56.21           ││  IP: 192.168.56.22           ││  IP: 192.168.56.23           │
│                              ││                              ││                              │
│ • Port 6443 (API Server)     ││ • Port 6443 (API Server)     ││ • Port 6443 (API Server)     │
│ • Port 9345 (Cluster Join)   ││ • Port 9345 (Cluster Join)   ││ • Port 9345 (Cluster Join)   │
│ • Roles: control-plane,      ││ • Roles: control-plane,      ││ • Roles: control-plane,      │
...
rke2-cp1       Ready    control-plane,etcd,master   22h     v1.30.4+rke2r1
rke2-cp2       Ready    control-plane,etcd,master   16h     v1.30.4+rke2r1
rke2-cp3       Ready    control-plane,etcd,master   15h     v1.30.4+rke2r1
rke2-worker1   Ready    <none>                      4h16m   v1.30.4+rke2r1
```

## etcd-snapshot

```sh
root@rke2-cp1:/var/lib/rancher/rke2/server/db/snapshots# ls
etcd-snapshot-rke2-cp1-1786809603  etcd-snapshot-rke2-cp1-1786845605  etcd-snapshot-rke2-cp1-1786860005
etcd-snapshot-rke2-cp1-1786838402  etcd-snapshot-rke2-cp1-1786852805
```

## 高可用演練 vagrant halt cp1

```sh
Aug 16 09:29:07 rke2-lb haproxy[4359]: [WARNING]  (4359) : Server rke2_api_back/rke2-cp1 is DOWN, reason: Layer4 timeout, info: " at initial connection step of tcp-check", check duration: 2002ms. 2 active and 0 backup servers left. 1 sessions active, 0 requeued, 0 remaining in queue.

# API Server 通過 LB 的存取正常
kubectl get nodes
NAME           STATUS     ROLES                       AGE     VERSION
rke2-cp1       NotReady   control-plane,etcd,master   24h     v1.30.4+rke2r1
rke2-cp2       Ready      control-plane,etcd,master   18h     v1.30.4+rke2r1
rke2-cp3       Ready      control-plane,etcd,master   18h     v1.30.4+rke2r1
rke2-worker1   Ready      <none>                      6h32m   v1.30.4+rke2r1
```

### grafana 顯示 cp1 cpu 使用歸0
